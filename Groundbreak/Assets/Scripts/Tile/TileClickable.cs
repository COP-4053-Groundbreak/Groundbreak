...
public class TileClickable : MonoBehaviour, IPointerDownHandler, IPointerEnterHandler, IPointerExitHandler
{
    ...
    public void clearEffectRangeIndicator(int range, Tile startTile, List<Tile> neighborsVisited){
        foreach (Tile t in ReactionManager.gridManager.grid){
            // In throw range
            if (Player.GetComponent<PlayerActions>().throwRange >= t.GetComponent<TileClickable>().GetDistance()){
    ...
    void IPointerExitHandler.OnPointerExit(PointerEventData eventData) 
    {
        ...
        if (turnLogic.isThrowPhase && lastTileHovered.transform.GetChild(0).GetComponent<Renderer>().material.color != Color.white){
            clearEffectRangeIndicator(effectIndicatorRange, lastTileHovered, new List<Tile>());
        } 
