...
public class changeAnimationPumpkin : MonoBehaviour {
	...
	public int numberOfAnimations = 7;
	int actualAnimation;
	...
	void OnGUI(){
		...
		if (GUI.Button (new Rect (Screen.width *0.1f, Screen.height * 0.8f, Screen.width * 0.15f, Screen.height * 0.1f), "Change to next animation")) {
			changeAnimation ();
		}
	}
	...
	void changeAnimation(){
		...
		else if (actualAnimation == 6) {
			actualAnimation = 6;
			lockOtherAnimations ();
			animator.SetBool ("changeToAnim6", true);
		} 

		else if (actualAnimation == 7) {
			actualAnimation = 7;
			lockOtherAnimations ();
			animator.SetBool ("changeToAnim7", true);
		} 
	...
	void lockOtherAnimations(){
		...
		animator.SetBool ("changeToAnim6", false);
		animator.SetBool ("changeToAnim7", false);
	}
}
